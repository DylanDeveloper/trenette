...
public class TrenetteKenBurnsView extends FrameLayout {
    ...
    private List<ImageBundle>  mObjets;
    private TrenetteLoopViewPager mLoopViewPager;
    ...
    public TrenetteKenBurnsView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TrenetteKenBurnsView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setPager(TrenetteLoopViewPager mPager) {
    ...
    public void loadObjects(List<ImageBundle>  objects, TextView imageNameText, TextView imageDetailsText) {
        mLoadType               = LoadType.OBJECTS;
        mImageNameText          = imageNameText;
        mimageDetailsText       = imageDetailsText;
        sCachedSizeForLoadType  = 0;
        mObjets = objects;
        if (mRootLayout != null) {
            if(mImageViews != null) {
    ...
    private void loadImages(final int position, final int activeIndex) {
        ...
        if(position != prePosition && position != nextPosition) {
            if(mObjets != null) {
                mImageNameText.setText(mObjets.get(position).getName());
                mimageDetailsText.setText(mObjets.get(position).getDetails());
            }
        }
    ...
    private int getSizeByLoadType() {
        ...
        switch (mLoadType) {
            ...
            case MIXING:
                ...
                break;
            case OBJECTS:
                sCachedSizeForLoadType = mObjets.size();
                break;
        }
        return sCachedSizeForLoadType;
    }
    ...
    private void loadImage(final int position, final int imageIndex) {
        switch (mLoadType) {
            ...
            case MIXING:
                ...
                break;
            case OBJECTS:
//                Glide.with(mContext).load(mObjets.get(position).getImageURL()).into(mImageViews[imageIndex]);
                mImageViews[imageIndex].setImageBitmap(mObjets.get(position).getBitmap());
              break;
        }
    }
